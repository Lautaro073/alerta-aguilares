...
UPDATE reports
SET priority = CASE
  WHEN category IN ('ACCIDENTE', 'SEMAFORO', 'ALUMBRADO') THEN 'high'
  WHEN category IN ('BACHE', 'SENALIZACION', 'VEHICULO_ABANDONADO') THEN 'medium'
  ELSE 'low'
END
...
DROP FUNCTION IF EXISTS create_report_with_rate_limit(
  TEXT,
  DOUBLE PRECISION,
  DOUBLE PRECISION,
  TEXT,
...
BEGIN
  ...
  SELECT * INTO v_fp FROM rate_limits WHERE id = 'fp:' || p_fingerprint_hash FOR UPDATE;
  SELECT * INTO v_ip FROM rate_limits WHERE id = 'ip:' || p_ip_hash FOR UPDATE;
